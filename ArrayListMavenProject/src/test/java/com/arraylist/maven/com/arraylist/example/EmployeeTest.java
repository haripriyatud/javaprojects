...
public class EmployeeTest {
	...
	public void employees_with_salary_greaterthan500000() {
		List<String> employee_names_result = employee_list.stream().filter(e->e.getSalary()>500000).map(e -> e.getName()).collect(Collectors.toList());
		assertThat(employee_With_salary_greater_than500000, is(employee_names_result));
	}
	
	@Test
	public void system_engineer_employees() {
		List<String> employee_names_result = employee_list.stream().filter(e->e.getDesignation().contains("System Engineer")).map(e -> e.getName()).collect(Collectors.toList());
		assertThat(system_enginers, is(employee_names_result));
	}
	
	@Test
	public void increment_10percent_for_work_experience_greater_than_5() {
		List<Double> incremented_salary = employee_list.stream().filter(e->e.getYearsExperience()>5).map(e -> e.getSalary() + (e.getSalary()*0.10)).
